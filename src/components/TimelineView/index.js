...
import CoureTimeLineCard from '../CourseTimelineCard'
...
class TimelineView extends Component {
  render() {
    const Project = timelineItemsList.filter(
      each => each.categoryId === 'PROJECT',
    )

    const Course = timelineItemsList.filter(
    ...
    return (
      <div>
        ...
        <Chrono mode="VERTICAL">
          {Course.map(each => (
            <CoureTimeLineCard projectDetails={each} />
          ))}
